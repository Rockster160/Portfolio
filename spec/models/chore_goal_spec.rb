...
RSpec.describe ChoreGoal do
  ...
  describe "kinds — current_value, reached?, refresh!" do
    ...
    describe ":chore_completions" do
      it "counts only completions of its configured chore" do
        ...
        goal = ChoreGoal.create!(
          user: user, name: "Drink 3", kind: :chore_completions,
      ...
      it "relative snapshots the baseline so an existing count doesn't pre-fill progress" do
        ...
        create(:chore_completion, user: user, chore: chore)
        goal = ChoreGoal.create!(
          user: user, name: "Drink 100", kind: :chore_completions,
    ...
    describe ":chore_streak" do
      it "cumulative uses max(current_streak, longest_streak)" do
        ...
        goal = ChoreGoal.create!(
          user: user, name: "Streak", kind: :chore_streak,
      ...
      it "relative ignores streaks that started before the goal was created" do
        ...
        ChoreStreak.create!(
          user: user, chore: chore, current_streak: 5,
  ...
  describe "refresh! semantics" do
    ...
    it "is idempotent — re-running on an achieved goal is a no-op" do
      goal = ChoreGoal.create!(
        user: user, name: "Easy", kind: :pebbles,
    ...
    it "is a no-op when the goal hasn't been reached" do
      goal = ChoreGoal.create!(
        user: user, name: "Hard", kind: :pebbles,
  ...
  describe "completed goals stay at 100%" do
    let(:chore) { create(:chore, created_by_user: user, reward_pebbles: 5) }

    it "progress_percent locks at 100 once achieved, even if balance is later withdrawn" do
      goal = ChoreGoal.create!(
      ...
      create(:chore_completion, user: user, chore: chore, paid_pebbles: 12)
      expect(goal.refresh!).to be(true)
      expect(goal.progress_percent).to eq(100)

      # Withdraw everything — the displayed progress must NOT regress.
      create(:chore_withdrawal, user: user, amount_pebbles: 50)
      expect(goal.reload.progress_percent).to eq(100)
      expect(goal.current_value).to eq(goal.target_value)
      expect(goal.reached?).to be(true)
    end

    it "non-achieved goals continue to compute progress normally" do
      goal = ChoreGoal.create!(
        user: user, name: "Saver", kind: :pebbles,
        scope_mode: :cumulative, tracking_mode: :earned, target_value: 100
      )
      create(:chore_completion, user: user, chore: chore, paid_pebbles: 25)
      expect(goal.progress_percent).to eq(25)
    end
  end
end
