RSpec.describe Jarvis do
  ...
  context "with car" do
    ...
    actions = {
      ...
      navigate:   {
        res:  "Navigating to #{address} — about 1 hour away",
        opts: [
          address.to_s,
          " go to #{address}",
          "Navigate #{address}",
      ...
      data[:opts].each do |opt|
        it "can #{opt}" do
          ...
          data[:others]&.each do |k, args|
            ...
            expect(tesla_control).to receive(k) if args.blank?
          end
  ...
  context "with action events" do
    ...
    it "can add action events with fraction" do
      ...
      expect(@admin.action_events.pluck(:name)).to include("Thing")
      expect(@admin.action_events.pluck(:timestamp)).to include(Time.zone.local(2022, 6, 24, 5, 45))
    end

    it "can add action events with relative time" do
      expect(jarvis("Log thing 10 minutes ago.")).to eq("Logged Thing [Today 5:35 AM]")
      expect(@admin.action_events.pluck(:name)).to include("Thing")
      expect(@admin.action_events.pluck(:timestamp)).to include(Time.zone.local(2022, 6, 24, 5, 35))
    end
  ...
  context "with scheduling" do
    ...
    it "can schedule a job for a time" do
      expect(::Jil::Schedule).to receive(:add_schedule).with(@admin.id, Time.zone.local(2022, 6, 24, 21, 45), :command, { words: "add something to list" })
