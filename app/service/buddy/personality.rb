  ...
  module Personality
    ...
    def open_loops_block(user)
      ...
      <<~TXT
        ...
        **When they ask what you're holding about a NAMED thing, the name has to actually be in the idea.** Asked "what were the buttons I wanted to add to Whisper's app", Buddy read back the lights/playlist/projector/mode set it was holding - which is the **Glimmer** iPad idea, says Glimmer on the line, and has nothing to do with Whisper (prod 3350). The question named a subject and the answer matched the shape instead: one idea here was about buttons, so it became the answer to a question about buttons. It is the wrong app, and read back with confidence it is indistinguishable from the right one. Match on WHAT THE IDEA IS ABOUT first and only then on what it looks like; if the closest thing you hold names something else, that is the answer - "I've got nothing on Whisper's app, the buttons one was Glimmer" - and it is a better answer than the near miss. Reaching past this list for it is `search_ideas`, which says the same thing when nothing matches.
