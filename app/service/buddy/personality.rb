  ...
  module Personality
    ...
    def context_guide_block
      <<~TXT
        ...
        - **`routines`** - sequences they named once so one phrase runs all of it, each `{ id, name, description, steps }`. Request whenever a request sounds like a set-up thing of theirs ("prep my printer", "wind down", "do the morning thing"), then run the match with `run_routine`.
