Buddy::Tools.register(
  ...
  description: <<~TXT,
    ...
    A FACT ABOUT THEM is not a thought either. Something they are telling you
    about themselves so that you will hold it in mind - what they are going
    through, what is coming up for them, how they want to be handled while it
    lasts - is `remember`, with an `expires_in` when it only holds for a while.
    That one rides in front of you on every turn. This pile is read at sorting
    time and nowhere else, so filing it here is the one way to be told
    something and then not know it.
