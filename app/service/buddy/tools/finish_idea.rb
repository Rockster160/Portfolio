Buddy::Tools.register(
  ...
  confirm:     ->(payload, ctx) {
    ...
    { summary: "Tick this off?", resolved: { label: idea.summary.presence || idea.content.to_s.first(60) } }
  },
  ...
  execute:     ->(payload, ctx) {
    ...
    before = { "status" => idea.status, "relevant_at" => idea.relevant_at }
    idea.update!(status: :done, relevant_at: nil)
    {
      idea_id: idea.id,
      label:   payload[:label],
      revert:  { op: "updated", model: "BuddyMemory", id: idea.id, before: before, summary: "put that back on your pile" },
    }
  },
