class BowlingStats
  ...
  def split_data(bowler)
    ...
    counts = grouped.each_with_object({}) { |((pins, spare), count), obj|
      obj[pins] ||= [0, 0]
      obj[pins][0] += count if spare
      obj[pins][1] += count
    }.sort_by { |pins, (_picked, _total)| pins.length }.sort_by { |_pins, (_picked, total)| -total }
  ...
  def missed_drink_frames
    ...
    set_game_frames.each_with_object({}) { |(set_id, game_num, frame_num), obj|
