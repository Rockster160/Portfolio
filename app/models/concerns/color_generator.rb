...
module ColorGenerator
  class << self
    def fade(from_hex, to_hex, steps=10, fade_back=false)
      steps ||= 10
      steps = steps.to_i
      raise ColorGenerationError, "Cannot fade more than 256 steps!" if steps > 256
      ...
      raise ColorGenerationError, "Start color must be a Hex value!" unless [from_red, from_green, from_blue].compact.length == 3

      to_red, to_green, to_blue = hex_to_rgb(end_hex)
      raise ColorGenerationError, "End color must be a Hex value!" unless [to_red, to_green, to_blue].compact.length == 3
    ...
    def lighten_pure_hex(hex, percent)
      raise ColorGenerationError, "Cannot lighten non-hex!" unless hex.length.in?(1..2)
    ...
    def lighten(full_hex, percent)
      ...
      raise ColorGenerationError, "Cannot lighten non-hex!" unless hex_without_hash.length.in?([3, 6])
    ...
    def rgb_to_hex(rgb)
      r, g, b = rgb.map { |val|
        ...
        new_val.length == 1 ? "0#{new_val}" : new_val
      }
      "##{r}#{g}#{b}".upcase
    end
    ...
    def conform_hex(hex_try)
      ...
      raise ColorGenerationError, "Hex values can only be characters A-F and numbers 0-9" if new_hex =~ /[^a-f0-9]/i
      raise ColorGenerationError, "Colors must be valid 3 or 6 character Hex value." unless [3, 6].include?(new_hex.length)

      new_hex = new_hex.chars.map { |single_hex| "#{single_hex}#{single_hex}" }.join if new_hex.length == 3
      "##{new_hex}"
    end
    ...
    def contrast_text_color_on_background(background_hex)
      ...
      return white if background_hex.blank?

      color_hex = background_hex.gsub("#", "")
      return white unless [6, 3].include?(color_hex.length)
