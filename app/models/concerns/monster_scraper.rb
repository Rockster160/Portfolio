...
module MonsterScraper
  class << self
    def scrape(_opts={})
      ...
      Monster.where(name: nil).find_each(&:reload_data)
    end

    def update_monster_data(monster)
      setup_capybara
      return if monster.try(:url).blank?
      ...
      skill_containers.each do |skill_container|
        ...
        multiplier = "" if description == multiplier
    ...
    def wait_for_element(element_selector, max_time=60)
      start = Time.zone.now
      print "Waiting"
      until page.all(element_selector).any? || (Time.zone.now > start + max_time.seconds)
        print "."
        sleep 0.5
      end
      puts "Found in #{(Time.zone.now - start).to_f.round(2)} seconds"
    end
    ...
    def awakened_monster_urls
      page.all("#bestiary_table tbody tr").map { |tr|
