class MapGenerator
  ...
  def set_relative_grass_cells(x, y)
    ...
    @relative_grass_cells = {
      up_left:    up_left_grass,
      up:         up_mid_grass,
      up_right:   up_right_grass,
      left:       mid_left_grass,
      mid:        mid_mid_grass,
      right:      mid_right_grass,
      down_left:  down_left_grass,
      down:       down_mid_grass,
      down_right: down_right_grass,
    }
  end
  ...
  def check_relative_grass(all_present, not_allowed)
    ...
    not_allowed += (all - all_present) if not_allowed.include?(:all)
  ...
  def generate_grass_for_cell(x, y)
    ...
    if check_relative_grass([:down, :left, :down_left], [:mid])
      "grass-down-left-inner-corner"
    elsif check_relative_grass([:down, :right, :down_right], [:mid])
      "grass-down-right-inner-corner"
    elsif check_relative_grass([:up, :left, :up_left], [:mid])
      "grass-up-left-inner-corner"
    elsif check_relative_grass([:up, :right, :up_right], [:mid])
      "grass-up-right-inner-corner"

    elsif check_relative_grass([:down_left], [:mid, :down, :left])
      "grass-down-left-corner"
    elsif check_relative_grass([:down_right], [:mid, :down, :right])
      "grass-down-right-corner"
    elsif check_relative_grass([:up_left], [:mid, :up, :left])
      "grass-up-left-corner"
    elsif check_relative_grass([:up_right], [:mid, :up, :right])
      "grass-up-right-corner"

    elsif check_relative_grass([:right], [:mid, :left])
      "grass-left-edge"
    elsif check_relative_grass([:left], [:mid, :right])
      "grass-right-edge"
    elsif check_relative_grass([:up], [:mid, :down])
      "grass-down-edge"
    elsif check_relative_grass([:down], [:mid, :up])
      "grass-up-edge"

    elsif check_relative_grass([:mid], [])
      rand_grass
    else
      "space"
    end
  ...
  def div(klasses, body=nil, attribute_hash={})
    ...
    attributes = attribute_hash.map { |attribute_key, attribute_val|
