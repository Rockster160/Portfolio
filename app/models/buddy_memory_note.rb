...
#  buddy_memory_id :bigint           not null
#
class BuddyMemoryNote < ApplicationRecord
  belongs_to :buddy_memory
  ...
  # BuddyMemory#waiting_label so a thread reads consistently whichever end you
  # look at it from.
  ...
  def touch_thread
    buddy_memory.update_columns(last_touched_at: created_at, updated_at: Time.current)
  end
end
