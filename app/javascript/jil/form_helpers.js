...
export function prettify(colorize, type, text) {
  ...
  if (type === "string") {
    ...
    ).replace(/\#\{([^\}]*?)\}/g, (_all, content) => {
      return prettify(
        colorize,
        "op",
        `#{${prettify(colorize, "variable", content)}}`,
      );
    });
  } else if (type == "spaces") {
...
export function field(arg) {
  if (!arg.typename) {
    if (arg.content) {
      ...
      let dynamicWrapper = element("span", {
        class: "hidden dynamic input-wrapper",
      });
      let selectedTag = element("span", { class: "selected-tag" });
      dynamicWrapper.appendChild(
        element("btn", {
          innerText: "< String >",
          data: { inputtype: "String", allowInput: false },
        }),
      );
  ...
  if (entryfield.tagName == "TEXTAREA") {
    ...
    wrapper.appendChild(
      element("btn", {
        innerText: "< Array >",
        data: { inputtype: arg.typename, allowInput: false },
      }),
    );
...
export function inputFromArg(arg) {
  ...
  if (typeof arg.placeholder !== "undefined") {
    ele.placeholder = arg.placeholder;
  }
  if (
    typeof arg.defaultval !== "undefined" &&
    arg.defaultval !== null &&
    arg.defaultval !== "null"
  ) {
...
export function inputFromType(typename) {
  ...
  let ele = element("input", {
    class: type == "password" ? "text-security" : null,
  });
  if (type == "password") {
...
export function select(opts, data) {
  ...
  opts.forEach((item) => {
    ele.appendChild(
      element("option", { name: item, value: item, innerText: item }),
    );
  });
  ele.appendChild(
    element("option", {
