...
Keyboard.on(["Enter", "Shift+Enter"], (evt) => {
  ...
  let refs = wrapper.querySelectorAll(
    evt.shiftKey ? ".content-dropdown" : ".content-dropdown.below",
  );
  if (refs.length == 0) {
    refs = wrapper.querySelectorAll(".reference");
  }
  if (statement) {
...
document.addEventListener("mousedown", function (event) {
  if (event.target.matches("a, .btn, input, .statement")) {
    return;
  }
  if (event.button === 1) {
    // middle click
...
document.addEventListener("click", function (evt) {
  if (
    evt.target.matches(".reference") ||
    evt.target.matches(".content-dropdown")
  ) {
    ...
    if (!evt.target.closest(".content-dropdown") && statement) {
      ...
      // Top-level reference, show global dropdown opts
      ...
      let globalOptions = function () {
        return Schema.globalMethods(!!content)
          .map((method) => {
...
document.addEventListener("click", function (evt) {
  if (evt.target.closest(".obj-refname")) {
    ...
    if (!other_ref) {
      return window.alert(
        "Reference not found - please enter an existing variable name",
      );
    }
    if (!Schema.method(other_ref, statement.method)) {
      return window.alert(
        `New reference type "${other_ref.returntype}" cannot call "${statement.method}".`,
      );
    }
...
document
  .querySelectorAll(".input-wrapper input[type=text], .input-wrapper textarea")
  .forEach((input) => showSpaces(input));
