...
export default class Statement {
  ...
  static toCode(pretty = false) {
    return Array.from(this.topLevelNodes())
      .map((statement) => {
  ...
  static find(id) {
    return this.all.find(
      (statement) => statement.id == id || statement.name == id,
    );
  }
  static from(element) {
  ...
  static nameTaken(name, ignore) {
    ...
    return Statement.all.some((statement) => {
      if (ignore && statement.id == ignore.id) {
        return false;
      }
  ...
  static regex(escaped_args) {
    ...
    let captArgs = escaped_args
      ? /(?<args>__TOKEN\d+__)/
      : /\((?<args>[\s\S]*)\)/; // First to last
  ...
  static fromText(text) {
    ...
    [...text.matchAll(/^ *\*?(?<varname>[_a-z][_0-9A-Za-z]*) *=/gm)].forEach(
      (m) => {
  ...
  static available(btn, statement) {
    ...
    while (previous) {
      [previous, ...previous.querySelectorAll(".statement-wrapper")].forEach(
        (prev) => {
          let prevStatement = Statement.from(prev);
          if (prevStatement) {
            if (
              types.indexOf("Any") >= 0 ||
              types.indexOf(prevStatement.returntype) >= 0
            ) {
  ...
  downReferences() {
    return Statement.all
      .map((statement) => (statement.refId == this.node.id ? statement : null))
      .filter(Boolean);
  }
  updateReferences() {
  ...
  set reference(ref) {
    ...
    this.node
      .querySelector(".obj-refname")
      .classList.toggle("hidden", this.scope == "singleton");
    this.node
      .querySelector(".obj-type")
      .classList.toggle("hidden", this.scope == "instance");
    this.validate();
  }
  validate() {
    this.clearError();
  ...
  get name() {
    return this.node.querySelector(".obj-varname").innerText;
  }
  set name(newname) {
    if (!newname) {
      newname = "";
  ...
  get returntype() {
    return this.node.querySelector(".obj-returntype").innerText;
  }
  set returntype(new_type) {
    this._returntype = new_type;
    this.node.querySelector(".obj-returntype").innerText =
      new_type == "Global" ? "Any" : new_type;
    if (new_type == "None") {
      this.node
        .querySelectorAll(".obj-varname, .obj-eq")
  ...
  get refname() {
    return this.node.querySelector(".obj-refname").innerText;
  }
  set refname(new_ref) {
    this.node.querySelector(".obj-refname").innerText = new_ref;
  }
  get type() {
    return this._type || this.node.querySelector(".obj-type").innerText;
  }
  set type(new_type) {
    this._type = new_type;
  ...
  get method() {
    return this.node.querySelector(".obj-method").innerText;
  }
  ...
  set inspect(bool) {
    this._inspect = bool;
    this.node
      .querySelector(".obj-inspect")
      .classList.toggle("fa-eye", this.inspect);
    this.node
      .querySelector(".obj-inspect")
      .classList.toggle("fa-eye-slash", !this.inspect);
    this.updateReferences();
  }
  toggleInspect() {
  ...
  dropdownOpts() {
    ...
    let copy = {
      icon: fa("regular clipboard"),
      title: "Copy to Clipboard",
      callback: async () => navigator.clipboard.writeText(this.toString()),
    };
    let pasteup = {
      text: "↑",
      icon: fa("paste regular"),
      title: "Paste Above",
      callback: () => this.pasteAbove(),
    };
    let pastedown = {
      text: "↓",
      icon: fa("paste regular"),
      title: "Paste Below",
      callback: () => this.pasteBelow(),
    };
    let comment = {
      icon: fa("hashtag"),
      title: "Toggle Comment",
      callback: () => this.toggleComment(),
    };

    return [
      // [dup, copy, pasteup, pastedown, comment],
      ...[
        ...Schema.types["Object"].instances,
        ...Schema.instancesFor(this.returntype),
      ].map((method) => {
        return {
          text: `#${method.name}`,
  ...
  showDropdown() {
    Dropdown.show(this.dropdownOpts());
  }
  ...
  set argString(str) {
    ...
    let inputs = Array.from(
      argsContainer.querySelectorAll(
        ":scope > .input-wrapper, :scope > .content",
      ),
    );
      ...
      if (wrapper.classList.contains("content")) {
        ...
        if (
          ...
          if (selectedTag) {
            ...
            let dynamicWrapper = wrapper.querySelector(
              ".dynamic.input-wrapper",
            );
            dynamicWrapper.classList.remove("hidden");
            dynamicWrapper.querySelector(":scope > .selected-tag").innerText =
              val;
          }
        } else {
              ...
              try {
                const parsed = val.replace(
                  ...
                  (all, slashes, char, midx) => {
                    ...
                    return (
                      "\\".repeat(odd ? length - 1 : length) + (char || "")
                    );
                  },
                );
  ...
  argValue(arg, nest, pretty, passComment) {
    ...
    tag =
      tag || arg.querySelector(":scope > .dynamic > .selected-tag")?.innerText;
    if (tag) {
      return prettify(color, "variable", tag);
    }
    let inputSelector = ["input", "textarea", "select"]
      .map((type) => `:scope > ${type}`)
      .join(", ");
    ...
    if (arg.classList.contains("content")) {
      let statements = Array.from(
        arg.querySelectorAll(":scope > .statement-wrapper"),
      );
    ...
    if (!input && arg.querySelector(":scope > .selected-tag")) {
      return prettify(color, "string", JSON.stringify(""));
    }
    if (!input) {
      return;
    } // Words (And, DO, IF, etc...)
    switch (input.tagName.toLowerCase()) {
      ...
      case "input":
        switch (input.type) {
          case "number":
            if (input.value === "") {
              return prettify(color, "numeric", "null");
            }
            return prettify(
              color,
              "numeric",
              JSON.stringify(parseFloat(input.value)),
            );
          case "checkbox":
            return prettify(color, "constant", JSON.stringify(input.checked));
          default:
            return prettify(
              color,
              "string",
              JSON.stringify(input.dataset.raw || input.value),
            );
        }
      default:
