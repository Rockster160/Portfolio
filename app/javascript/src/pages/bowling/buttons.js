...
export function buttons() {
  ...
  onEvent("click", ".shot", function (evt) {
    ...
    let bowlerNum = parseInt(
      this.closest(".bowler").getAttribute("data-bowler"),
    );
  ...
  onKeyDown("ArrowLeft", ":not(input), .shot", function (evt) {
    ...
    if (bowler) {
      ...
      if (!newShot) {
        ...
        if (!newFrame) {
          newFrame = bowler.frames[10];
        }
        newShot = newFrame.shots[newFrame.shots.length - 1];
      }
      game.currentShot =
        bowler.frames[newFrame.frameNum].shots[newShot.shotNum];
    }
  });
  ...
  onEvent("mousedown touchstart", function (evt) {
    if (touchInterface && evt.type == "mousedown") {
      return;
    }
  ...
  let mouseDownEvt = function (xPos, yPos) {
    ...
    let pinWrapper = document
      .elementFromPoint(xPos, yPos)
      .closest(".pin-wrapper");
    if (!pinWrapper) {
      return;
    }
    if (pinWrapper.matches(".fallen-before")) {
      return;
    }

    game.pinTimer.freeze();
    if (pinKnocking == undefined) {
      // undefined is when we haven't clicked a pin yet
  ...
  onEvent("mousedown mousemove mouseover", function (evt) {
    if (touchInterface) {
      return;
    } // Don't trigger mouse events if on a touch screen
    if (evt.target.tagName == "INPUT") {
      return;
    }
    if (evt.which != 1) {
      return;
    } // return unless holding left click
