...
import {
  ...
  multiKeyUp,
  isKeyPressed,
} from "../../support/multi_key_detection";
...
$(document).ready(function () {
  if ($(".ctr-little_worlds.act-show").length == 0) {
    return;
  }
  ...
  screenLog = function () {
    ...
    if (lastBlockHoveredCoord.length == 2) {
      $(".screen-log .block-coord").html(
        lastBlockHoveredCoord[0] + ", " + lastBlockHoveredCoord[1],
      );
    } else {
  ...
  postDestination = function () {
    ...
    if (timestamp < currentPlayer.lastMoveTimestamp) {
      return;
    }
  ...
  scrollToPlayer = function () {
    ...
    var startLeft = $(window).scrollLeft(),
      newLeft =
        playerPos.left - $(window).width() / 2 + littleWorld.blockWidth / 2;
    var startTop = $(window).scrollTop(),
      newTop = playerPos.top - $(window).height() / 2;

    scrollLeftDiff = newLeft - startLeft;
    if (scrollLeftDiff > maxScrollSpeed) {
      scrollLeftDiff = maxScrollSpeed;
    }
    if (scrollLeftDiff < -maxScrollSpeed) {
      scrollLeftDiff = -maxScrollSpeed;
    }
    ...
    if (scrollTopDiff > maxScrollSpeed) {
      scrollTopDiff = maxScrollSpeed;
    }
    if (scrollTopDiff < -maxScrollSpeed) {
      scrollTopDiff = -maxScrollSpeed;
    }
    ...
    chatBoxTimer = setTimeout(function () {
      $(".messages-container").animate(
        ...
        },
        {
          duration: duration + 1,
          complete: function () {
            $(".messages-container").addClass("hidden").css("opacity", 1);
          },
        },
      );
    }, delay + 1);
