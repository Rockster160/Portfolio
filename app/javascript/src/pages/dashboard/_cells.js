...
Cell.register = function (init_data) {
  ...
  cell.autocomplete_options =
    init_data.autocomplete_options || cell.command_list;
...
Cell.init = function (cell, config) {
  ...
  if (cell.onload && typeof cell.onload === "function") {
    Cell.current = cell;
    try {
      cell.onload();
    } finally {
  ...
  if (new_text == undefined) {
    ...
    new_text = new_text
      ...
      .map(function (line) {
        ...
        return Text.fixHeight(
          '<div class="line ' + wrap_class + '">' + (line || " ") + "</div>",
        );
      })
      .join("");
...
Cell.prototype.markWSConnected = function (connected) {
  this.ele.addClass("has-ws");
  this.ele.toggleClass("ws-disconnected", !connected);
};
...
Cell.prototype.command_list = function () {
  var commands = [".reload", ".debug", ".start", ".stop", ".hide", ".show"];
...
Cell.prototype.autocomplete = function (text) {
  ...
  if (
    ...
    typeof this.autocomplete_options === "function"
  ) {
...
Cell.prototype.execute = function (text) {
  if (this.my_command && typeof this.my_command === "function") {
    this.my_command(text);
  }
...
Cell.prototype.stop = function () {
  ...
  if (cell.stopped && typeof cell.stopped === "function") {
    cell.stopped();
  }
};
Cell.prototype.start = function () {
  ...
  if (cell.started && typeof cell.started === "function") {
    cell.started();
  }
  cell.reload();
};
...
Cell.prototype.active = function (reset_omnibar) {
  if (!$(this.ele).hasClass("livekey")) {
    Cell.blur();
  }
  $(".dash-cell").removeClass("active");
  this.ele.addClass("active");
  if (this && this.onlook && typeof this.onlook === "function") {
    this.onlook();
  }
  if (!reset_omnibar) {
    return;
...
Cell.blur = function () {
  ...
  if (cell && cell.onblur && typeof cell.onblur === "function") {
    cell.onblur();
  }
};
...
Cell.startLivekey = function () {
  ...
  if (cell && cell.livekey && typeof cell.livekey === "function") {
    ...
    if (cell.onfocus && typeof cell.onfocus === "function") {
      cell.onfocus();
    }
  }
};
...
CellWS = function (cell, init_data) {
  ...
  cell_ws.socket.onopen = function () {
    cell_ws.open = true;
    cell_ws.cell.markWSConnected(true);
  ...
  cell_ws.socket.onclose = function () {
    cell_ws.open = false;
    cell_ws.reload = true;
    cell_ws.cell.markWSConnected(false);
    if (init_data.onclose && typeof init_data.onclose === "function") {
      init_data.onclose.call(cell_ws);
    }
  };
