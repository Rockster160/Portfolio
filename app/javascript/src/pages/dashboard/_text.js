...
Text.overlay = function (top_text, bottom_text) {
  ...
  var new_text_arr = [];
  var top_chars = top_text.padEnd(length, " ").split(""),
    bot_chars = bottom_text.padEnd(length, " ").split("");
...
Text.link = function (url, body, plain) {
  ...
  return "[" + (plain ? "plain " : "") + (body || url) + "](" + url + ")";
};
...
Text.progressBar = function (percent, opts) {
  ...
  opts.open_char = Text.animate(
    opts.hasOwnProperty("open_char") ? opts.open_char : "[",
  );
  opts.progress_char = Text.animate(
    opts.hasOwnProperty("progress_char") ? opts.progress_char : "=",
  );
  opts.current_char = Text.animate(
    opts.hasOwnProperty("current_char") ? opts.current_char : ">",
  );
  opts.empty_char = Text.animate(
    opts.hasOwnProperty("empty_char") ? opts.empty_char : " ",
  );
  opts.close_char = Text.animate(
    opts.hasOwnProperty("close_char") ? opts.close_char : "]",
  );
  opts.post_text = opts.hasOwnProperty("post_text")
    ? opts.post_text
    : (function (pc) {
  ...
  progress_chars = clamp(
    progress_chars - opts.current_char.length,
    0,
    opts.width,
  );
...
Text.escapeSpecial = function (text) {
  ...
  var subbed_text = text.replaceAll(
    joined_regex,
    function (found, found_idx, full_str) {
...
Text.markup = function (text) {
  ...
  text = text.replaceAll(
    /\[plain (.*?)\]\((.*?)\)/gi,
    '<a href="$2" target="_blank" rel="noopener noreferrer" class="dash-link dash-link-plain">$1</a>',
  );
  text = text.replaceAll(
    /\[((?!plain )[^\]]*?)\]\((.*?)\)/gi,
    '<a href="$2" target="_blank" rel="noopener noreferrer" class="dash-link">$1</a>',
  );
