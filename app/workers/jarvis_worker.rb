class JarvisWorker
  ...
  def perform(user_id, msg)
    ...
    case parsed
    when String
      puts "\e[36m[LOGIT] | parsed is a string:[#{parsed.class}]#{parsed}\e[0m"
      ::Jarvis.command(User.find(user_id), parsed)
    when Hash
      puts "\e[36m[LOGIT] | parsed is a hash:[#{parsed.class}]#{parsed}\e[0m"
      event_data = parsed[:event]
      if event_data[:user_id].present? && event_data[:type].present?
        puts "\e[36m[LOGIT] | calling trigger:[#{parsed.class}]#{parsed}\e[0m"
        ::Jarvis.trigger(
          event_data[:type],
