class WebhooksController < ApplicationController
  ...
  # How long a press stays claimed. Long enough to cover any caller that keeps
  # retrying, short enough that a button pressed twice on purpose still needs
  # two distinct `pressed_at` values, which it always has.
  TRIGGER_REPLAY_WINDOW = 15.minutes
  ...
  def jil
    if params.key?(:trigger)
      ...
      data = json_params[:data].presence || json_params.except(:trigger).deep_merge(from_scope)
      jil_trigger(scope, data) unless replayed_trigger?(scope, data)
    else
      json_params.each do |trigger, data|
        jil_trigger(trigger, data) unless replayed_trigger?(trigger, data)
      end
    end
  ...
  def push_notification_subscribe
    ...
    # One row per DEVICE. The endpoint is NOT the device — it's the device's
    # current address, and iOS issues a new one every time it revokes a
    # subscription, which it does whenever a push arrives that shows no
    # notification. Keyed on the endpoint alone, every revocation filed a brand
    # new device: 21 live rows for one person across three actual devices, of
    # which two were real and nineteen were addresses nothing listens to. The
    # fan-out is serial, so those nineteen were nineteen timeouts standing in
    # front of the phone that mattered.
    #
    # `device_id` is generated by the client and kept in localStorage, so the
    # same install keeps one row and its endpoint is rewritten in place. A
    # client that doesn't send one (an older bundle, a cleared localStorage)
    # falls back to matching on the endpoint, which is what this did before and
    # is still right for a device whose address hasn't moved.
    scope    = current_user.push_subs.for_channel(channel)
    push_sub = (device_id.present? ? scope.find_by(device_id: device_id) : nil)
    push_sub ||= scope.find_by(endpoint: endpoint)
    push_sub ||= current_user.push_subs.new(channel: channel)
    push_sub.assign_attributes({
      endpoint:      endpoint,
      device_id:     device_id || push_sub.device_id,
  ...
  # shape straight through. The Timers PWA builds its body by hand and sends
  # them flat, which raised NoMethodError on nil here and 500'd every attempt —
  # which is why the timers channel has never had a single subscription row.
  def push_subscription_keys
    nested = params.permit(keys: [:auth, :p256dh])[:keys]
    return nested.slice(:auth, :p256dh).to_h.symbolize_keys if nested.present?

    params.permit(:auth, :p256dh).to_h.symbolize_keys
  end
  ...
  # The same event arriving again, rather than a second one.
  #
  ...
  # without one are not deduped - there is nothing to key on, and guessing at
  # an identity from the whole body would swallow a second genuine event that
  # happened to look the same.
  #
  # `unless_exist` is SET NX on Redis, so the claim is atomic and two requests
  # in flight together cannot both take it.
  def replayed_trigger?(scope, data)
    return false unless data.is_a?(Hash)

    stamp = data[:pressed_at].presence
    return false if stamp.blank?

    key = ["jil_trigger", scope, data[:device_id], data[:type], stamp].join(":")
    !Rails.cache.write(key, true, expires_in: TRIGGER_REPLAY_WINDOW, unless_exist: true)
  end
