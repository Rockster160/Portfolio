class ActionEventsController < ApplicationController
  ...
  def update
    ...
    ::Jil.trigger(@event.user_id, :event, @event.legacy_serialize.merge(action: :changed))
  ...
  def destroy
    ...
    if event.destroy
      ::Jil.trigger(event.user_id, :event, event.legacy_serialize.merge(action: :removed))
    else
